...
def _mk_adapters(board) -> Tuple:
    ...
    def apply_move(b, m: MoveMeta):
        ...
        if fn: 
            fn(m.from_sq, m.to_sq, promotion=m.promotion); 
            return
            ...
            def _w(*a, **kw):
                ...
                try:
                    ...
                    if mode == "san":
                        ...
                        if len(a) >= 2:
                            from_sq, to_sq = str(a[0]), str(a[1])
                    if san is None and hasattr(board, "last_san"):
                        try:
                            san = board.last_san()
                        except Exception:
                            san = None
                    history.push(board, MoveMeta(san=san or "?", 
                                                 from_sq=from_sq or "?", 
                                                 to_sq=to_sq or "?", 
                                                 promotion=promo))
                except Exception:
...
def attach_replay(app, snapshot_stride: int = 8):
    ...
    history.reset(board)
    _wrap_apply_for_logging(board, history)
    ...
    # --- 보드 교체 감시 & 자동 재바인딩 ---
    last_board = {"obj": board}

    def _refresh_binding():
        try:
            cur = _resolve_board(app)
        except Exception:
            app.after(200, _refresh_binding)
            return

        if cur is not last_board["obj"]:
            last_board["obj"] = cur
            gs, ss, am = _mk_adapters(cur)
            history.get_state = gs
            history.set_state = ss
            history.apply_move = am
            history.reset(cur)
            _wrap_apply_for_logging(cur, history)
        app.after(150, _refresh_binding)

    app.after(150, _refresh_binding)

    # --- 새 게임/불러오기 류 메서드 감싸서 즉시 재바인딩 트리거 ---
